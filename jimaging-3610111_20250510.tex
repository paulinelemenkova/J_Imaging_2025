...
\Author{Polina %MDPI: Please carefully check the accuracy of names and affiliations. % Author: yes, correct.
 Lemenkova %MDPI: 1.If only one author, correspondence is not needed, so we deleted it, please confirm. 2. If only one affiliation, affiliation number is not needed, so we deleted it, please confirm. % Author: 1. yes, I confirm. 2. yes, I confirm. 
 \orcidA{}}
...
\address[1]{%
 Department %MDPI: We arranged the authors’ address information from subordinate to superior. Please check and confirm this modification. % Author: yes, agree and confirm. 
 of Biological, Geological and Environmental Sciences, Alma Mater Studiorum---Università di~Bologna, Via Irnerio 42, 40126 Bologna, Italy; polina.lemenkova2@unibo.it; Tel.: +39-3446928732.  %MDPI: 1. Please confirm if Emilia-Romagna can be deleted since it's unnecesary. 2. Please confirm if both emails should be kept. % Author: 1. yes, can be deleted (I deleted it). 1. yes, 2nd email is not needed, I removed it. 
...
Traditional agro-silvo-pastoral practices that once maintained a rich landscape mosaic have declined throughout the 20th century due to rural depopulation and socio-economic shifts~\cite{Vitali}. Social factors include the aging of farmers and the relocation of young people to larger urban centres for better economic conditions~\cite{MAREGGI2024103358,BRUNORI2007183}. This affected agricultural and agroforestry sectors in Apennines and caused changes in landscape mosaic, in a similar fashion to what observed in other mountainous landscapes~\cite{LASANTA2017810,PERPINACASTILLO2021104946,geomatics5010005,Arnaez}. The widespread agricultural land abandonment resulted in forest expansion and habitat homogenization. Over the 1936--2018 period, for instance, forest cover increased from 29\% (1,673,266 %MDPI: Please use commas to separate out the thousands in numbers with five or more digits (but not four digits). Please confirm if the period should be comma. The same below. % Author: yes, I confirm (corrected to commas). 
 ha) to 38\% (2,214,671 ha) in Central Italy~\cite{AGNOLETTI2022119655}, and the area has been characterized as one of the land use change hotspots of Europe~\cite{Kuemmerle_2016}. While some of these processes have positive effects, such as ecosystem restoration and increased forest cover, others pose significant challenges, including biodiversity loss, soil erosion, and reduced landscape diversity~\cite{Falcucci,AMEZTEGUI2021104240}.
...
\begin{figure}[H]
     ...
     \caption{Topographic %MDPI: Please provide an explanation for the red star abd blue square % Author: information added.
 map showing the location of the study area. Blue square indicates the location of the Landsat satellite images within Italy; red star denotes the position of the capital. Map source: authors. Software: Generic Mapping Tools (GMT) {version 6.5.0.}}
  \label{fig01}
\end{figure}
...
%\end{adjustwidth}
\caption{Original %MDPI: Please try to straighten the figure as much as possible here and in the figures below % Author: yes, corrected (deleted "adjustwidth" for better position).
...
\bottomrule
\end{tabularx}
\noindent{\footnotesize{Information source: USGS. These %MDPI: Footnotes are not supported in our journal. We have therefore included this paragraph in the table footer. Please confirm. Same case for below table footer % Author: yes, I confirm. 
...
\begin{adjustwidth}{-\extralength}{-5cm}
\centering 
	\begin{subfigure}[b]{.25\textwidth}
		\centering
...
\begin{table}[H]
\caption{Training ground truth samples of land types from CORINE.}\label{tab02}%
\small
\begin{tabularx}{\textwidth}{lll}
\toprule
\textbf{Landsat OLI/TIRS Land Cover Class} & \textbf{CORINE Codes} & \textbf{Number of Pixels}  \\
\midrule
        Artificial surfaces & 111--142 & 1,274,084 \\ \midrule
        Agricultural areas & 211--244 & 2,236,395  \\ \midrule
        Broadleaved forests & %MDPI: Please confirm if a space should be added after comma to separate the numbers in this column. % Author: I removed extra numbers (typesetting errors signifying RGM codes - unnecessary here). I sorted rows by CORINE Codes (increasing).
311, 313--335 & 857,355  \\ \midrule
        Coniferous forests & 312 & 4,543,992  \\ \midrule
        Mixed forests & 313 & 47,634,298  \\ \midrule
...
Since classification maps invariably contain misclassified pixels and possible classification mistakes, accuracy assessment is an essential step in any remote sensing-based classification effort. Besides, we compared the performance of four classification algorithms (RF, SVM, DT and {the} MLPC): Table \ref{tab04}, Table \ref{tab05}, Table \ref{tab06} and Table \ref{tab07}. Numerous factors cause possible errors, including the following ones:
\begin{itemize} 
	...
	\item the hierarchical level of the separability categories (10 classes), Appendix \ref{secMatr}.
\end{itemize} 
...
{Cartographic accuracy of image classification is presented on the series of rejection probability maps. These maps visualise the reject raster maps which indicate the reject threshold results. The maps are generated based on the computed chi %MDPI: Please confirm if the italics are necessary; if not, please remove them.  % Author: not necessary, I removed it.
 square test on each image analysis at various thresholds of confidence. This maps determines at what confidence level each cell is classified correctly. The visualised maps are presented in Figure} \ref{figA2} as the reject threshold layer which contains the index to one calculated confidence level for each classified cell in each Landsat image. The results of image classification were validated against CORINE Land Cover (CLC) data using GRASS GIS to evaluate the accuracy. The choice of CORINE is explained as follows. The CORINE CLC is a pan-European land cover database, offering a detailed classification scheme. The technical details, including metadata reference are available publicly on the Copernicus website: %MDPI: We cannot access the link to this website. Please check and provide a available website. % Author: information updated.
 \url{https://land.copernicus.eu/en} (accessed on 10 May 2025). %MDPI: Please provide the date you accessed the URL in the following format: “URL (accessed on Day Month Year)”. % Author: information added.


\begin{figure}[H]

\begin{adjustwidth}{-\extralength}{-4.5cm}
\centering 
	\begin{subfigure}[b]{.25\textwidth}
...
\end{adjustwidth}
\caption{Accuracy %MDPI: Please add a left bracket in the image, e.g., “A)” should be “(A)”. % Author: corrected.
...
Clustering-based classification demonstrated the overall accuracy of the obtained maps for convergence of 98.1\%, 98.2\% and 98.1\%, respectively. This demonstrated high mapping accuracy based on GRASS GIS processing of {RS} data. Among the accuracy methods, }Kappa presents a discrete multivariate {approach}, {Table }\ref{tab03}. The evaluation of Kappa is normally suggested as moderate for values in the interval 0.41--0.60, acceptable for values from 0.61 to 0.80 and almost perfect agreement for values above 0.81. Spatial uncertainty in image classification shows the distribution of pixels to target classes which affects mapping {results}. It was estimated to avoid bias by the \emph{chi} square test which plots the reject raster map. This map shows the level of threshold which determines{ }confidence level at which each cell is categorised correctly and accurately assigned to the {given} class. 

In the study area, 10 distinct land cover types {were considered, as} summarised in Table~\ref{tab02}. {Their} CORINE codes, number of cells (pixels) in Landsat scenes and{ }number of training samples {were sued }for ML validation, Figure \ref{figA2}. Nevertheless, the thematic aggregation of these 10 land cover classes cannot be directly linked to the parameters of vegetation and requires further studies{. For} RF algorithm, the number of correctly classified pixels (shown in bold black %MDPI: Please check if this explanation applies for the bold in all the tables below % Author: yes, correct for Tables 4, 5, 6 and 7.
 in Table~\ref{tab04}) is 35,720. Divided by the total number of points 37,440, the overall classification accuracy for RF approach is {97}\%. The same approach was used for evaluation of classification correctness by other algorithms. 

\begin{table}[H] 
\caption{{F-1 %MDPI: Please confirm if the table has no table header. If not, please add bold for table header. % Author: the table has table header: "F-1 Score of Predictive Performance and Cohen's Kappa...".
 Score of Predictive Performance and Cohen's Kappa Predictive Agreement for ML models in GRASS GIS: (1) Random Forest (RF), (2) Decision Tree, (3) Multilayer Perceptron Classifiers (MLPClassifier), (4) Support Vector Machine (SVM)}.\label{tab03}}
...
Tables \ref{tab04}--\ref{tab07} show the error matrices that report the report the numerical data{ }for the results of classification according to four tested algorithms{.}The tables show the relationship between the classified pixels in the satellite image and the corresponding ground truth data in the CORINE{.} {According to the error matrices presented Tables} \ref{tab04}--\ref{tab07}, the ML demonstrated robust accuracy for satellite image classification with high accuracy with the best performance is shown by the SVM algorithm slightly followed by the RF, DT and MLPC approaches. 

\begin{table}[H]

\caption{Error %MDPI: 1. We removed all vertical lines from the tables, please confirm. 2. Please confirm whether to keep current format of table (text direction, no header and bold formatting). If the bold should be kept, please provide an explanation for it. These comments also applies to Tables 5-8. % Author: 1. yes, I confirm. 2. yes, text direction is for length of vertical cells. Bold is deleted. 
...
\begin{tabularx}{\textwidth}{ p{1.3cm}  X  X  X  X  X  X  X  X  X  X  p{0.6cm}  p{0.8cm} }
  \toprule
 \vspace{3pt}  Category & \rotatebox{90}{1: urban areas}\vspace{-24pt} & \rotatebox{90}{2: arable lands}\vspace{-24pt} & \rotatebox{90}{3: water areas}\vspace{-24pt} & \rotatebox{90}{4: needle-leaved} \vspace{-24pt}& \rotatebox{90}{5: bare lands}\vspace{-24pt} & \rotatebox{90}{6: pastures}\vspace{-24pt} & \rotatebox{90}{7: herbaceous}\vspace{-24pt} & \rotatebox{90}{8: croplands}\vspace{-24pt} & \rotatebox{90}{9: broadleaved}\vspace{-24pt}  & \rotatebox{90}{10: shrubland}\vspace{-24pt} & \vspace{-15pt}\rotatebox{90}{Row Total} &\vspace{-24pt} \rotatebox{90}{Correct Pixels}\vspace{9pt} \\
   \midrule
    Class 1 & {2589} & 48 & 62 & 1 & 0 & 50 & 2 & 18 & 0 & 2 & 2772 & 2589\\
    Class 2 & 3 & {6359} & 27 & 0 & 5 & 1 & 1 & 2 & 0 & 0 & 6398 & 6359 \\    
    Class 3 & 1 & 0 & {2908} & 1 & 32 & 47 & 0 & 0 & 10 & 5 & 3004 & 2908 \\    
    Class 4 & 2 & 0 & 17 & {4818} & 9 & 7 & 14 & 0 & 2 & 0 & 4818 &  4869\\    
    Class 5 & 0 & 4 & 0 & 11 & {6138} & 4 & 3 & 0 & 0 & 0 & 6160 & 6138 \\ 
    Class 6 & 0 & 2 & 1 & 0 & 3 & {4281} & 12 & 21 & 0 & 0 & 4390 & 4281 \\ 
     Class 7 & 14 & 0 & 0 & 5 & 18 & 10 & {1407} & 9 & 28 & 22 & 1513 & 1407 \\     
     Class 8 & 0 & 0 & 14 & 3 & 0 & 1 & 2 & {3389} & 0 & 9 & 3418 & 3389\\  
     Class 9 & 1 & 2 & 0 & 0 & 0 & 31 & 12 & 0 & {2409} & 0 & 2421 & 2409\\   
     Class 10 & 0 & 0 & 2 & 3 & 5 & 9 & 14 & 0 & 0 & {2513} & 2546 & 2513\\ 
     \midrule 
     {Total} & 2600 & 6415 & 3031 & 4842 & 6200 & 4431 & 1517 & 3439 & 2439 & 2569 & {37,440} & 36,862\\ 
    \bottomrule
\end{tabularx}
\end{table}
...
\caption{{Error} matrix for validation of {Random Forest} (RF) algorithm.}
\label{tab05}
\begin{tabularx}{\textwidth}{ p{1.3cm}  X  X  X  X  X  X  X  X  X  X  p{0.6cm}  p{0.8cm} }
  \toprule
  \vspace{3pt} Category & \rotatebox{90}{1: urban areas}\vspace{-24pt} & \rotatebox{90}{2: arable lands}\vspace{-24pt} & \rotatebox{90}{3: water areas}\vspace{-24pt} & \rotatebox{90}{4: needle-leaved} \vspace{-24pt}& \rotatebox{90}{5: bare lands}\vspace{-24pt} & \rotatebox{90}{6: pastures}\vspace{-24pt} & \rotatebox{90}{7: herbaceous}\vspace{-24pt} & \rotatebox{90}{8: croplands}\vspace{-24pt} & \rotatebox{90}{9: broadleaved}\vspace{-24pt}  & \rotatebox{90}{10: shrubland}\vspace{-24pt} & \vspace{-15pt}\rotatebox{90}{Row Total} &\vspace{-24pt} \rotatebox{90}{Correct Pixels}\vspace{9pt} \\
   \midrule
    {Class 1} & {2175} & 73 & 51 & 0 & 42 & 72 & 12 & 27 & 6 & 7 & 2465 & 2175\\
    {Class 2} & 8 & {5886} & 25 & 14 & 31 & 18 & 0 & 0 & 1 & 4 & 5987 & 5886 \\
    {Class 3} & 0 & 0 & {2924} & 5 & 27 & 35 & 0 & 12 & 0 & 9 & 3012 & 2924 \\  
    {Class 4} & 0 & 0 & 19 & {5102} & 4 & 0 & 2 & 15 & 13 & 0 & 5155 & 5102 \\ 
    {Class 5} & 0 & 1 & 0 & 12 & {5957} & 4 & 11 & 25 & 1 & 0 & 6011 & 5957\\
    {Class 6} & 0 & 12 & 43 & 0 & 0 & {3984} & 24 & 0 & 0 & 1 & 4064 & 3984 \\
     {Class 7} & 3 & 8 & 1 & 0 & 0 & 0 & {3106} & 12 & 17 & 32 & 3179 & 3106 \\
     {Class 8} & 0 & 5 & 1 & 17 & 28 & 0 & 0 & {2396} & 0 & 31 & 2478 & 2396\\
     {Class 9} & 0 & 0 & 15 & 9 & 23 & 8 & 39 & 45 & {1965} & 10 & 2114 & 1965\\
     {Class 10} & 2 & 3 & 0 & 0 & 0 & 28 & 15 & 2 & 0 & {2925} & 2975 & 2925\\ 
     \midrule 
     {Total} & 2144 & 6208 & 3247 & 5154 & 6117 & 4393 & 1474 & 3485 & 2523 & 2649 & {37,440} & 36,420\\ 
    \bottomrule
\end{tabularx}
\end{table}
...
\caption{{Error} matrix for validation of {Decision Tree }(DT) algorithm.}
\label{tab06}
\begin{tabularx}{\textwidth}{ p{1.3cm}  X  X  X  X  X  X  X  X  X  X  p{0.6cm}  p{0.8cm} }
  \toprule
  \vspace{3pt} Category & \rotatebox{90}{1: urban areas}\vspace{-24pt} & \rotatebox{90}{2: arable lands}\vspace{-24pt} & \rotatebox{90}{3: water areas}\vspace{-24pt} & \rotatebox{90}{4: needle-leaved} \vspace{-24pt}& \rotatebox{90}{5: bare lands}\vspace{-24pt} & \rotatebox{90}{6: pastures}\vspace{-24pt} & \rotatebox{90}{7: herbaceous}\vspace{-24pt} & \rotatebox{90}{8: croplands}\vspace{-24pt} & \rotatebox{90}{9: broadleaved}\vspace{-24pt}  & \rotatebox{90}{10: shrubland}\vspace{-24pt} & \vspace{-15pt}\rotatebox{90}{Row Total} &\vspace{-24pt} \rotatebox{90}{Correct Pixels}\vspace{9pt} \\
   \midrule
    {Class 1} & {2137} & 63 & 71 & 0 & 0 & 80 & 1 & 21 & 0 & 1 & 2373 & 2137\\
    {Class 2} & 5 & {6142} & 32 & 1 & 3 & 2 & 0 & 0 & 2 & 4 & 6191 & 6142 \\
    {Class 3} & 0 & 0 & {3115} & 3 & 36 & 42 & 0 & 2 & 12 & 8 & 3218 & 3115 \\
    {Class 4} & 0 & 0 & 23 & {5116} & 6 & 8 & 12 & 0 & 1 & 0 & 5166 & 5116 \\
    {Class 5} & 0 & 2 & 0 & 15 & {6012} & 8 & 1 & 2 & 1 & 0 & 6041 & 6012\\
    {Class 6} & 0 & 0 & 4 & 0 & 58 & {3235} & 9 & 2 & 0 & 55 & 3363 & 3235 \\
     {Class 7} & 1 & 0 & 2 & 0 & 36 & 8 & {1398} & 0 & 1 & 15 & 1461 & 1398 \\
     {Class 8} & 0 & 1 & 0 & 2 & 3 & 4 & 0 & {3457} & 2 & 12 & 3481 & 3457\\
     {Class 9} & 1 & 0 & 0 & 12 & 6 & 3 & 32 & 0 & {2551} & 0 & 2605 & 2551\\
     {Class 10} & 0 & 0 & 0 & 5 & 7 & 3 & 21 & 1 & 0 & {2604} & 2641 & 2604\\ 
     \midrule 
     {Total} & 2144 & 6208 & 3247 & 5154 & 6117 & 4393 & 1474 & 3485 & 2523 & 2649 & {37,440} & 35,720\\ 
    \bottomrule
\end{tabularx}
...
\caption{{Error} matrix for validation of {Multi-layer Perceptron (MLP) Artificial Neural Network (ANN)}~algorithm.}
\label{tab07}
\begin{tabularx}{\textwidth}{ p{1.3cm}  X  X  X  X  X  X  X  X  X  X  p{0.6cm}  p{0.8cm} }
  \toprule
 Category & \rotatebox{90}{1: urban areas}\vspace{-15pt} & \rotatebox{90}{2: arable lands}\vspace{-15pt} & \rotatebox{90}{3: water areas}\vspace{-15pt} & \rotatebox{90}{4: needle-leaved} \vspace{-12pt}& \rotatebox{90}{5: bare lands}\vspace{-15pt} & \rotatebox{90}{6: pastures}\vspace{-15pt} & \rotatebox{90}{7: herbaceous}\vspace{-15pt} & \rotatebox{90}{8: croplands}\vspace{-15pt} & \rotatebox{90}{9: broadleaved}\vspace{-15pt}  & \rotatebox{90}{10: shrubland}\vspace{-15pt} & \vspace{-24pt}\rotatebox{90}{Row Total} &\vspace{-27pt} \rotatebox{90}{Correct Pixels}\vspace{12pt} \\
   \midrule
    {Class 1} & {2095} & 68 & 63 & 2 & 0 & 15 & 0 & 0 & 1 & 12 & 2256 & 2095\\
    {Class 2} & 2 & {6371} & 38 & 0 & 0 & 21 & 14 & 0 & 1 & 0 & 6447 & 6371 \\
    {Class 3} & 0 & 2 & {3487} & 2 & 18 & 27 & 0 & 0 & 0 & 12 & 3548 & 3487 \\
    {Class 4} & 3 & 2 & 1 & {5279} & 2 & 9 & 24 & 0 & 0 & 0 & 5320 & 5279 \\
    {Class 5} & 0 & 40 & 13 & 39 & {5364} & 0 & 0 & 22 & 0 & 11 & 5489 & 5364\\
    {Class 6} & 0 & 0 & 5 & 1 & 13 & {4106} & 1 & 14 & 21 & 0 & 4161 & 4106 \\
     {Class 7} & 5 & 0 & 47 & 12 & 34 & 0 & {1405} & 0 & 31 & 62 & 1596 & 1405 \\
     {Class 8} & 0 & 9 & 23 & 0 & 0 & 1 & 2 & {3188} & 0 & 4 & 3227 & 3188\\
     {Class 9} & 15 & 10 & 2 & 41 & 20 & 4 & 22 & 0 & {2356} & 0 & 2470 & 2356\\
     {Class 10} & 0 & 0 & 3 & 0 & 0 & 31 & 14 & 0 & 2 & {2376} & 2426 & 2376\\ 
     \midrule 
     {Total} & 2144 & 6208 & 3247 & 5154 & 6117 & 4393 & 1474 & 3485 & 2523 & 2649 & {37,440} & 36,127\\ 
    \bottomrule
\end{tabularx}
\end{table}
...
The comparison of the four algorithms was performed using self-statistics based on the test data, as well as error matrices based on comparison with Corine Land cover data, Figure \ref{fig06}. The main land cover types identified included following 10 categories: (1)~%MDPI: Newly added information, please confirm. % Author: yes, I confirm.
...
\begin{figure}[H]
     ...
     \caption{Land %MDPI: Please add a left bracket in the image, e.g., “A)” should be “(A)”. % Author: corrected.
...
More specifically, between 2018 and 2023, the total agricultural surface in the study area decreased by approximately 8.371,%MDPI: Please use periods as decimal signs instead of commas in whole text. % Author: corrected.
5 ha ($-$16.65\%), according to the data analysis from the {ML} methods of data processing. The areas occupied by the Class 2 (``arable lands and agricultural fields'') have decreased by 19.8 \% from 3.%MDPI: We changed comma to decimal period, please confirm. The same below. % Author: yes, I confirm. 
77 to 2.64  ha which shows the minor decrease of cultivated and arable surfaces during this period. The extension of woodland is notable during the five-year period. In fact, according to the cartographic data, the difference between the areas occupied by forest and shrubland (Class  9 ``broadleaved trees'' and Class 10 ``shrubland'') in 2018 and 2023 shows the slight increase of scrubland and shrubs, as well as broadleaved forests. Furthermore, woodland areas detected in the image analysis suggest that the woodland cover becomes greater. 



{The areas covered by these land cover types were calculated and compared using four ML approaches to assess land cover dynamics. Using calculations of the landscape fragments, the regression and progression of forest areas were then assessed and reported in results of tabular form that show which type of the land cover types is expanding or decreasing. } {Comparative performance of algorithms is additionally evaluated using F-1 Score of Predictive Performance and Cohen's Kappa Predictive Agreement were computed for ML models in GRASS GIS: (1) Random Forest (RF), (2) Decision Tree, (3) Multilayer Perceptron Classifiers (MLPClassifier),(4) Support Vector Machine (SVM), Table }\ref{tab03}. The cartographic analysis shows that Class 8 represented by 8) croplands in the study area decreased by 36.7\% while the Class 7 ``permanent herbaceous vegetation'' increased by 17.5\%, Figure \ref{fig09}. 

The results indicate that the use of this algorithm coupled to the 30~m resolution multispectral bands of Landsat had the best performance compared to other machine learning models, respectively. Seasonal variations of pixels by 10 classes on 2019, 2022 and 2023 are summarised in Table~\ref{tab08}. 
\begin{figure}[H]
  
      \includegraphics[width=0.99\textwidth]{Fig_09.jpg} \\
     \caption{Satellite %MDPI: Please add a left bracket in the image, e.g., “A)” should be “(A)”. and revise the overlapped content on the legend % Author: corrected.
...
\begin{tabularx}{\textwidth}{ p{1.2cm}  X  X  X  X  X  X  X  X  X  X  }
  \toprule
   Spring & \rotatebox{90}{1: urban areas} & \rotatebox{90}{2: arable lands} & \rotatebox{90}{3: water areas} & \rotatebox{90}{4: needle-leaved} & \rotatebox{90}{5: bare lands} & \rotatebox{90}{6: pastures} & \rotatebox{90}{7: herbaceous} & \rotatebox{90}{8: croplands} & \rotatebox{90}{9: broadleaved} & \rotatebox{90}{10: shrubland}  \\
   \midrule
    2019 & 404 & 477 & 818 & 850 & 781 & 980 & 813 & 965 & 610 & 229 \\
    2022 & 398 & 433 & 981 & 775 & 732 & 892 & 900 & 963 & 521 & 240 \\
    2023 & 557 & 473 & 912 & 815 & 667 & 1128 & 955 & 900 & 622 & 291 \\\midrule
   Autumn %MDPI: Sub-tables are not recommended in our journal. We have merged all sub-tables into one table. Please confirm. % Author: yes, I confirm. 
 & \rotatebox{90}{1: urban areas} & \rotatebox{90}{2: arable lands} & \rotatebox{90}{3: water areas} & \rotatebox{90}{4: needle-leaved} & \rotatebox{90}{5: bare lands} & \rotatebox{90}{6: pastures} & \rotatebox{90}{7: herbaceous} & \rotatebox{90}{8: croplands} & \rotatebox{90}{9: broadleaved} & \rotatebox{90}{10: shrubland} \\
   \midrule
    2019 & 680 & 342 & 725 & 645 & 854 & 682 & 622 & 629 & 858 & 313 \\
    2022 & 680 & 382 & 723 & 675 & 911 & 700 & 698 & 653 & 802 & 241 \\
    2023 & 720 & 341 & 696 & 673 & 1060 & 791 & 587 & 821 & 720 & 203 \\
    \bottomrule
\end{tabularx}
\end{table}

Technical differences in the algorithms of image processing are commented as follows. We compared the outputs of the classified images using algorithms of Support Vector Machine (SVM), Multi-layer Perceptron (MLP) Classifier of the Artificial Neural Network (ANN), Decision Tree Classifier (DTC), and Random Forest (RF), Figure \ref{fig10}. The uncertainties across the several ML techniques examined in this study (RF, SVN, DT, and MLPC) nearly always produced results that were acceptable, indicating that the analysis had a high degree of ML reliability, Figure \ref{fig10}. The best results are achieved by the SVM Classifier followed by the RF. Both algorithms differ from the DTC and ANN approaches, due to the different approaches in  identification of the pixels's value on the raster scenes (MLP of ANN and DTC). The maps generated using the SVM method are comparable with those based on the Random Forest Classifier, while MLP Classifier presented a more generalised map using deep learning network approach. The DTC demonstrated lesser quality of image processing compared to the above methods. 
...
\begin{figure}[H]
     ...
     \caption{Satellite %MDPI: 1. Please cite this figure in the text and ensure that the first citation of each figure appears in numerical order. 2. Please add a left bracket in the image, e.g., “A)” should be “(A)”. % Author: 1. Figure 10 cited above. 2. Figure quality is improved (bracket added).
...
The consequences of various ML parameters on the cartographic outputs are analysed in this work, such as speed and accuracy, randomness of nodes, analytical determination of the output weights, and dependence distribution of pixels for each algorithm. Supervised learning models of GRASS GIS were tested and compared. The results shown that the most time-consuming algorithms was noted as SVC classification, while the fastest results were achieved by the Decision Tree approach to image processing and the best results are achieved by RF Classifier. Though each algorithms was developed to serve different objectives of ML applications in RS data processing, their technical implementation and practical purposes present valuable approaches to cartographic data processing and image~analysis. %MDPI: The Author Contributions section was removed, as it is unnecessary when there is only one author. Please confirm. % Author: yes, I confirm. 

\vspace{6pt}
%----------- section ----------->


\funding{The APC was funded by Multidisciplinary Digital Publishing Institute (MDPI). %MDPI: Please add: ``This research received no external funding'' or ``This research was funded by NAME OF FUNDER grant number XXX.'' and  and ``The APC was funded by XXX''. Check carefully that the details given are accurate and use the standard spelling of funding agency names at \url{https://search.crossref.org/funding}, any errors may affect your future funding. % Author: information added.
}
...
\dataavailability{The Landsat data used in this study are freely available from the USGS repository (\url{https://earthexplorer.usgs.gov/} (accessed on 05 April 2025)%MDPI: Please provide the date you accessed the URL in the following format: “URL (accessed on Day Month Year)”. % Author: information added.
). The GEBCO data used for plotting topographic map in Figure \ref{figDia1} is freely available in GEBCO repository (\url{https://www.gebco.net/} (accessed on 03 April 2025)). The CORINE dataset is obtained from the Copernicus website (CORINE: %MDPI: We revised the format of weblink, please confirm. % Author: yes, I confirm.
 \url{https://land.copernicus.eu/en/products/corine-land-cover} (accessed on 07 April 2025)). The codes used for data processing are available in the GitHub repository of the author: \url{https://github.com/paulinelemenkova/Apennines} (accessed on 08 April 2025).} 
...
\appendixstart
\appendix
\section[\appendixname~\thesection]{Class %MDPI: 1. We formatted it as Appendix section, please confirm. If the section is the content of main text, please put it before funding section. 2. The Appendix was not mentioned in the main text. Please cite it in the main text. % Author: 1. yes, I confirm formatting. 2. The reference to Appendix is added in the main text, in section "Accuracy Assessment and Comparison of Four Algorithms". 
...
\end{equation}
\end{adjustwidth}
%MDPI: It is not recommended to add caption to the formula. Please confirm whether it can be changed to list format. The same below. % Author: yes, caption deleted and changed to list format (also below)
.\end{myequ}
\vspace{-12pt}
Class separability matrices between 10 classes: spring and autumn 2019.
...
\end{myequ}
\vspace{-12pt}
Class separability matrices between 10 classes: spring and autumn 2022.
...
\end{adjustwidth}
\end{myequ}
Class separability matrices between 10 classes: spring and autumn 2023.
